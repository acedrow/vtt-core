...
5. **Combat implementations via pack modules** — Named HP combat modules live in the content package and register on `CombatHookContribution.modules`. Shared engine code dispatches via local typed `combatMod(key)` helpers at call sites (not public barrel exports). Client UI helpers import `@vtt-core/hellpiercers-content/combat-ui`.
