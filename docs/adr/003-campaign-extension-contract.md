...
5. **`CharacterSheet.data` / `Player.data` bags** — Pack-owned sheet and live-player extras go under `data`. Content-specific loadout fields (including former top-level `yadathanTower`) must not be first-class engine keys.
6. **Optional `campaignHooks` on `ContentPack`** — `CampaignHookContribution` registers ensure/validate/apply for campaign WS actions plus optional sheet loadout extras. Hellpiercers implementations live in `@gaem/hellpiercers-content`; shared dispatches via [`campaign-hooks.ts`](../../packages/shared/src/campaign-hooks.ts) (no IP-named campaign facade modules).
7. **Deferred** — Versioned KV migrators and pack `id`+`version` on room/sheet state (parent area #7). Combat protocol union shrink (parent area #3).
