...
3. **Nested `GameState.campaign`** — Runtime fields (`partyResources`, `factionStates`, `mapRegions` / `mapParty` / `mapLocations` / `mapConvoys`, `gmResource`, `unlockedUpgrades`) live under typed `CampaignRuntimeState` on `GameState.campaign`. `normalizeGameState` runs `liftLegacyCampaignFields` (top-level → nested, dual-reading old and new key names) then `migrateCampaignRuntimeKeys` (old nested names → new; prefer new when both present; delete old). Stamped state emits only new keys. Call sites use shared campaign-state accessors only. One content semver dual-reads; after production rooms have cycled, a later semver may drop old-key reads.
